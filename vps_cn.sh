...
#
#  Portainer 采用官方默认安装：首次访问网页时自行创建管理员账号。
#  注意：容器启动后约 5 分钟内完成创建；超时则 docker restart portainer 重试。
# =============================================================================
...
EXTRA_SSH_USERS="${EXTRA_SSH_USERS:-}"               # 额外允许 SSH 的已有用户，空格分隔
...
# apt 升级 openssh-server 会重启/停止 ssh 服务、清掉该目录——
...
EOF
  ...
  # apt 升级 openssh-server 会清掉 /run/sshd，必须在这里重建，否则后续 sshd 检测全部 fatal
  ensure_run_sshd
...
OPTS
  ...
  # 『管道 + awk 提前 exit』会让上游收到 SIGPIPE(141)，pipefail 下误杀脚本——变量化无此问题。
...
# ==================== 部署 Portainer（官方默认方式） ====================
log "部署 Portainer CE（官方默认方式：首次访问网页时创建管理员）..."
mkdir -p /opt/portainer

# 幂等：数据卷存在 → Portainer 已初始化过（管理员已创建），直接沿用；
# 不存在 → 全新部署，首次访问页面时自行创建管理员（约 5 分钟窗口）
PORTAINER_FRESH=1
if docker volume inspect portainer_data &>/dev/null; then
  PORTAINER_FRESH=0
  warn "检测到已存在的 portainer_data 卷，沿用现有 Portainer 数据与管理员。"
fi

# 清理旧版脚本（v3.x）生成的密码文件：--admin-password-file 机制已移除，文件已无用途
if [ -f /root/portainer_admin_password_hash ] || [ -f /root/portainer_initial_password.txt ]; then
  ...
  log "已清理旧版 Portainer 密码文件（预设密码机制已移除）。"
fi
...
#!/usr/bin/env bash
# 由 VPS 初始化脚本 v4.0 生成：限制 Docker 发布端口 9443 的来源。
...
if [ "$PORTAINER_UP" = "1" ]; then
  if [ "$PORTAINER_FRESH" = "1" ]; then
    echo "  Portainer: 全新部署，管理员待创建"
    echo "  ★ 打开 https://localhost:9443 创建 admin 账号（启动后约 5 分钟内完成）"
    echo "    若页面提示超时: docker restart portainer 后刷新页面即可重试"
  else
    echo "  Portainer: 沿用现有数据，用户名/密码为首次部署时设置"
  fi
  if [ "$PORTAINER_BIND" = "127.0.0.1" ]; then
